...
Atelier::Atelier():Royaume(3,3,0,0,0,0){
    setCout(3);
    setValeur(3);
}
...
std::string Atelier::obtenirNom() const {
    return "Atelier";
}


void Atelier::action(Joueur& joueur,Partie& p){
    p.getReserve().afficherReserve();
    std::cout << "------------------------------------------" << std::endl;
    std::cout << "Choisissez une carte qui peut coûter jusqu'à 4" << std::endl;
    std::cout << "------------------------------------------ "<< std::endl;
